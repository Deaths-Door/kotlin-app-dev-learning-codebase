...
object PreferenceExtensions{
    ...
    fun <T> SharedPreferences.getPreferenceValue(key:String, defaultValue:List<T>? = listOf()): ArrayList<*>? {
        val str = Gson().toJson(defaultValue)
        val data = this.getString(key,str)
        return Gson().fromJson(data,ArrayList::class.java)
    }
    ...
    fun SharedPreferences.changePreference(key:String, value:Float) = this.edit().putFloat(key,value).commit()
    fun SharedPreferences.changePreference(key:String, value:Long) = this.edit().putLong(key,value).commit()
    fun SharedPreferences.changePreference(key:String, value:Set<String>) = this.edit().putStringSet(key,value).commit()
